...
+ (JTCacheManager *)sharedInstance{
    ...
    dispatch_once(&onceToken, ^{
        cacheInstance = [[JTCacheManager alloc] init];
    });
...
- (id)init{
    ...
    if (self) {
        ...
        _operationQueue = dispatch_queue_create("com.dispatch.JTCacheManager", DISPATCH_QUEUE_SERIAL);
...
- (void)initCachesfileWithName:(NSString *)name{
    ...
    self.diskCachePath =[[self JTKitPath] stringByAppendingPathComponent:name];
